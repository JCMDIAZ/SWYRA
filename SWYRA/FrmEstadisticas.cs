    ...
    public partial class FrmEstadisticas : Form
    {
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            queryConst = "declare @fini datetime declare @ffin datetime " +
                         ...
                         "insert @detped SELECT dp.CVE_DOC, COUNT(CVE_ART) AS ARTICULOS, SUM(ISNULL(CAST(SURTIDO AS INT), 0)) AS ARTICULOS_SURTIDOS, " +
        ...
        private List<EstadisticasEmpleado> CargaEstSurt()
        {
            ...
            try
            {
                var query = queryConst +
                            ...
                            "JOIN( SELECT *, ISNULL(DATEDIFF(MINUTE, FECHASURTIR, ISNULL(FECHASURTIRBROCAS, FECHAEMPAQUE)), 0) TIEMPO FROM( " +
                            ...
                            "MIN(case when ESTATUSPEDIDO = 'EMPAQUE' then FECHAMOV end) FECHAEMPAQUE from PEDIDO_HIST ph join @ped pd on pd.cve_doc = ph.CVE_DOC " +
                            "WHERE ESTATUSPEDIDO IN('SURTIENDO', 'SURTIENDO BROCAS', 'EMPAQUE') GROUP BY ph.CVE_DOC) AS A " +
        ...
        private List<EstadisticasEmpleado> CargaEstBroc()
        {
            ...
            try
            {
                var query = queryConst +
                            ...
                            "JOIN( SELECT *, ISNULL(DATEDIFF(MINUTE, FECHASURTIRBROCAS, FECHAEMPAQUE), 0) TIEMPO FROM( " +
                            "select ph.CVE_DOC, MIN(case when ESTATUSPEDIDO = 'SURTIENDO' then FECHAMOV end) FECHASURTIR," +
                            "MIN(case when ESTATUSPEDIDO = 'SURTIENDO BROCAS' then FECHAMOV end) FECHASURTIRBROCAS," +
                            "MIN(case when ESTATUSPEDIDO = 'EMPAQUE' then FECHAMOV end) FECHAEMPAQUE  from PEDIDO_HIST ph join @ped pd on pd.cve_doc = ph.CVE_DOC " +
                            "WHERE ESTATUSPEDIDO IN('SURTIENDO', 'SURTIENDO BROCAS', 'EMPAQUE') GROUP BY ph.CVE_DOC) AS A " +
        ...
        private List<EstadisticasEmpleado> CargaEstEmpq()
        {
            ...
            try
            {
                var query = queryConst +
                            ...
                            "JOIN( SELECT *, ISNULL(DATEDIFF(MINUTE, FECHAEMPACANDO, FECHAREMISION), 0) TIEMPO FROM( " +
                            "select ph.CVE_DOC, MIN(case when ESTATUSPEDIDO = 'EMPACANDO' then FECHAMOV end) FECHAEMPACANDO, " +
                            "MIN(case when ESTATUSPEDIDO = 'REMISION' then FECHAMOV end) FECHAREMISION from PEDIDO_HIST ph join @ped pd on pd.cve_doc = ph.CVE_DOC " +
                            "WHERE ESTATUSPEDIDO IN('EMPACANDO', 'REMISION') GROUP BY ph.CVE_DOC) AS A ) AS T ON T.CVE_DOC = P.CVE_DOC group by p.EMPAQUETADOR_ASIGNADO, u.Nombre";
        ...
        private List<EstadisticasEmpleado> CargaEstEtiq()
        {
            ...
            try
            {
                var query = queryConst +
                            ...
                            "JOIN( SELECT *, ISNULL(DATEDIFF(MINUTE, FECHAGUIA, FECHATERMINADO), 0) TIEMPO FROM( " +
                            "select ph.CVE_DOC, MIN(case when ESTATUSPEDIDO = 'GUIA' then FECHAMOV end) FECHAGUIA, " +
                            "MIN(case when ESTATUSPEDIDO = 'TERMINADO' then FECHAMOV end) FECHATERMINADO from PEDIDO_HIST ph join @ped pd on pd.cve_doc = ph.CVE_DOC " +
                            "WHERE ESTATUSPEDIDO IN('GUIA', 'TERMINADO') /*and FECHAMOV between @fini and @ffin*/ GROUP BY ph.CVE_DOC) AS A " +
        ...
        private List<EstadisticasPedidos> CargaEstHigh()
        {
            ...
            {
                var query = queryConst +
                            "select distinct Top 10 p.CVE_CLPV, LTRIM(p.CVE_DOC) CVE_DOC, e.ESTATUSPEDIDO, c.NOMBRE Cliente, s.ARTICULOS_SURTIDOS, " +
                            "(s.ARTICULOS - s.ARTICULOS_SURTIDOS) ARTICULOS_PENDIENTES, s.PIEZAS_SURTIDAS, s.PIEZAS_PENDIENTES, t.TIEMPO " +
                            "from PEDIDO p JOIN @ped pd ON pd.cve_doc = p.CVE_DOC JOIN vw_estatuspedido e ON e.CVE_DOC = p.CVE_DOC " +
                            ...
                            "select ph.CVE_DOC, MIN(case when ESTATUSPEDIDO = 'AUTORIZACION' then FECHAMOV end) FECHAUTORIZACION, " +
                            "MIN(case when ESTATUSPEDIDO = 'TERMINADO' then FECHAMOV end) FECHATERMINADO from PEDIDO_HIST ph join @ped pd on pd.cve_doc = ph.CVE_DOC " +
                            "WHERE ESTATUSPEDIDO IN('AUTORIZACION', 'TERMINADO') GROUP BY ph.CVE_DOC) AS A ) AS T ON T.CVE_DOC = P.CVE_DOC order by t.TIEMPO desc, LTRIM(p.CVE_DOC)";
        ...
        private List<EstadisticasPedidos> CargaEstLow()
        {
            ...
            try
            {
                var query = queryConst +
                            ...
                            "select ph.CVE_DOC, MIN(case when ESTATUSPEDIDO = 'AUTORIZACION' then FECHAMOV end) FECHAUTORIZACION, " +
                            "MIN(case when ESTATUSPEDIDO = 'TERMINADO' then FECHAMOV end) FECHATERMINADO from PEDIDO_HIST ph join @ped pd on pd.cve_doc = ph.CVE_DOC " +
                            "WHERE ESTATUSPEDIDO IN('AUTORIZACION', 'TERMINADO') GROUP BY ph.CVE_DOC) AS A ) AS T ON T.CVE_DOC = P.CVE_DOC " +
                            "where e.ESTATUSPEDIDO = 'TERMINADO' order by t.TIEMPO desc, LTRIM(p.CVE_DOC)";
                dets = GetDataTable("DB", query, 7).ToList<EstadisticasPedidos>();
            }
            catch (Exception ex)
            {
